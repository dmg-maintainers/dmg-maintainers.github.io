---
# Display name
title: Yehan Weerasinghe

# Full Name (for SEO)
first_name: YEHAN DINETH
last_name: WEERASINGHE

# Is this the primary user of the site?
superuser: false

# Role/position
role: AI and Automation Engineer

# Organizations/Affiliations
organizations:
  - name: Freedom Finance Australia
    url: https://ffau.com.au/

# We need this to automatically populate the list of publications from the website.
#zbmath_id: ZB_MATH ID


# Short bio (displayed in user profile at end of posts)
# bio: A BIT OF A BIO 
 
interests:
  - Robotics
  - Mathematics
  - Producitivty Tools
education:
  courses:
    - course: Bachelor or Artificial Intelligence
      institution: Deakin University
      year: 2027
   

# Social/Academic Networking
# For available icons, see: https://docs.hugoblox.com/getting-started/page-builder/#icons
#   For an email link, use "fas" icon pack, "envelope" icon, and a link in the
#   form "mailto:your-email@example.com" or "#contact" for contact widget.
social:
  # - icon: globe
  #   icon_pack: fas
  #   link: 'https://YOURWEBSITEORBLOG'
  - icon: envelope
    icon_pack: fas
    link: 'mailto:ydinethw@gmail.com'
  # - icon: twitter
  #   icon_pack: fab
  #   link: https://twitter.com/TWITTER?
  # - icon: google-scholar
  #   icon_pack: ai
  #   link: https://scholar.google.com/SCHOLAR?
  - icon: github
    icon_pack: fab
    link: https://github.com/yehandineth
# Link to a PDF of your resume/CV from the About widget.
# To enable, copy your resume/CV to `static/files/cv.pdf` and uncomment the lines below.
# - icon: cv
#   icon_pack: ai
#   link: files/cv.pdf

# Enter email to display Gravatar (if Gravatar enabled in Config)
# email: 'YOUREMAIL@INSTITUTION'

# Highlight the author in author lists? (true/false)
highlight_name: false

# Organizational groups that you belong to (for People widget)
#   Set this to `[]` or comment out if you are not using People widget.
user_groups:
  - Undergrad Students
---
